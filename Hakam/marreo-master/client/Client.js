...
Meteor.startup(function() {
    ...
    currentCanvas = ReactiveVar(null);
    UI.registerHelper('currentCanvas', function() {
        return currentCanvas.get();
    });

    UI.registerHelper('size', function(height, width) {
        return this.height == height && this.width == width;
    });

    UI.registerHelper('wide', function() {
        return this.height < this.width;
    });

    UI.registerHelper('bigger', function(height, width) {
        return this.height > height && this.width > width;
    });
    ...
    UI.registerHelper('avatar', function() {
        try {
            if (Meteor.user().services.facebook) {
                return "http://graph.facebook.com/" + Meteor.user().services.facebook.id + "/picture?width=200&height=200";
                ...
                return "/img/placeholders/avatars/placeholder.jpg";
            }
        } catch (err) {
            console.log(err);
        }
    ...
    Meteor.users.find({
        ...
        added: function(id) {
            // document.querySelector('#toast').text = id + ' just came online';
            // document.querySelector('#toast').show();
        },
        removed: function(id) {
            // document.querySelector('#toast').text = id + ' just went offline';
            // document.querySelector('#toast').show();
        }
    });
                ...
                if ($(e.target).is('body')) {
                    ...
                    if (text[0] == "!") {
                        ...
                        if (command.match(/^background (.+)/)) {
                            ...
                            currentCanvas.get().setBackground(value);
                            return;
                        }
                    }
                    ...
                    if (text[0] == "=") {
                        ...
                        if (UserSession.get('autoenhance') === true) {
                            mx.current.enhance(text);
                        } else if (text !== mx.current.text) {
